...
export default function Login() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const errors = validateLoginPayload({ email: email.trim().toLowerCase(), password })
    if (errors.length > 0) {
    ...
    try {
      ...
      const toastState = {
        ...
        description: 'Welcome back to TripZenAI!',
  ...
  const sendOtp = async (event?: FormEvent) => {
    ...
    const emailResolved = forgotEmail.trim().toLowerCase()
    if (!emailRegex.test(emailResolved)) {
    ...
    try {
      const response = await apiFetch<{ detail: string }>('/api/auth/forgot-password/send-otp', {
        method: 'POST',
        body: JSON.stringify({ email: emailResolved }),
      })
      setForgotEmail(emailResolved)
